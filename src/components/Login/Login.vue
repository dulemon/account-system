...
export default {
  ...
  methods: {
    login: function () {
      ...
      post("/user/login", { name: this.account, password: this.password }).then((res) => {
    ...
    register: function () {
      ...
      if (!namePattern.test(this.account)) {
        Message.error({ message: '用户名必须由2-16位，字母、数字、下划线组成!' })
        return;
      }
      if (!passwordPattern.test(this.password)) {
        Message.error({ message: '密码必须8位字母、数字组成，不能含有非法字符!' })
        return;
      }

      post("/user/register", { name: this.account, password: this.password, isAdmin: 1 }).then((res) => {
        if (res.description === 'success') {
